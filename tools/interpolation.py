...
if __name__ == '__main__':
    ...
    data_root = '/public/home/cookie/anaconda3/envs/ocsort_env/OC_SORT/datasets/mot/train'#mot17_train
    # data_root = '/public/home/cookie/anaconda3/envs/ocsort_env/OC_SORT/datasets/dancetrack/val'
    # data_root = '/public/home/cookie/anaconda3/envs/ocsort_env/OC_SORT/datasets/kitti/testing'
...
# def write_results_kitti(filename, results):
#     """
#     保存为 KITTI 格式:
#     frame id -1 -1 -1 x1 y1 x2 y2 -1 -1 -1 -1000 -1000 -1000 -10
#     """
#     save_format = '{frame} {id} Car -1 -1 -1 {x1:.2f} {y1:.2f} {x2:.2f} {y2:.2f} -1 -1 -1 -1000 -1000\n'
#     with open(filename, 'w') as f:
#         for i in range(results.shape[0]):
#             frame, tid, x1, y1, x2, y2 = results[i, :6]
#             f.write(save_format.format(frame=int(frame), id=int(tid), x1=x1, y1=y1, x2=x2, y2=y2))


# def dti_kitti(txt_path, save_path, n_min=10, n_dti=20):
#     seq_txts = sorted(glob.glob(os.path.join(txt_path, '*.txt')))
#     for seq_txt in seq_txts:
...
#         # 用 genfromtxt 支持字符串列
#         data_raw = np.genfromtxt(seq_txt, dtype=str, delimiter=' ')
#         if data_raw.ndim == 1:
#             data_raw = np.expand_dims(data_raw, axis=0)

#         # 只保留必要列: frame, id, x1, y1, x2, y2
#         seq_data = np.zeros((data_raw.shape[0], 6), dtype=np.float64)
#         seq_data[:, 0] = data_raw[:, 0].astype(float)  # frame
#         seq_data[:, 1] = data_raw[:, 1].astype(float)  # id
#         seq_data[:, 2:6] = data_raw[:, 6:10].astype(float)  # x1,y1,x2,y2

#         min_id = int(np.min(seq_data[:, 1]))
#         max_id = int(np.max(seq_data[:, 1]))

#         seq_results = np.zeros((1, 6), dtype=np.float64)

#         for track_id in range(min_id, max_id + 1):
#             tracklet = seq_data[seq_data[:, 1] == track_id]
#             if tracklet.shape[0] == 0:
#                 continue

#             n_frame = tracklet.shape[0]
#             tracklet_dti = tracklet.copy()
#             if n_frame > n_min:
#                 frames = tracklet[:, 0]
#                 frames_dti = {}
...
#                 if len(frames_dti) > 0:
#                     data_dti = np.zeros((len(frames_dti), 6), dtype=np.float64)
#                     for n, (frame_id, bbox) in enumerate(frames_dti.items()):
#                         data_dti[n, 0] = frame_id
#                         data_dti[n, 1] = track_id
#                         data_dti[n, 2:6] = bbox
...
#         seq_results = seq_results[seq_results[:, 0].argsort()]
#         save_seq_txt = os.path.join(save_path, seq_name)
#         write_results_kitti(save_seq_txt, seq_results)
#         print(f"✅ Done: {seq_name}, total {seq_results.shape[0]} lines")
...
#     txt_path, save_path = sys.argv[1], sys.argv[2]
#     mkdir_if_missing(save_path)
#     dti_kitti(txt_path, save_path, n_min=10, n_dti=20)
